...
for consumers, providers in simulation.runs(epochs):
    ...
    consumer_list = {_c: [sum(_v[:v]) for v in range(len(_v))]
                     for _c, _v in consumers.items()}
    best_consumer = max(consumer_list.keys(),
                        key=lambda c: consumer_list[c][-1])
    profiler.switch("main-1", "main-2")

    producer_list = {_p: [sum(_v[:v]) for v in range(len(_v))]
                     for _p, _v in providers.items()}
    best_provider = max(producer_list.keys(),
                        key=lambda p: producer_list[p][-1])
    profiler.switch("main-2", "main-3")

    consumer_label_set = False
    provider_label_set = False

    average_producer = [sum(x) / len(producer_list.values())
                        for x in zip(*producer_list.values())]
    average_consumer = [sum(x) / len(consumer_list.values())
