    ...
    def itea(
        ...
        for t in range(1, T+1):
            ...
            for advisor in range(0, recommendations.shape[0]):
                loss_advisors.append(loss_function(real_outcome, recommendations[advisor]))
            # Update weights
            for advisor_i in range(0, recommendations.shape[0]):
                for provider_i in range(0, recommendations.shape[1]):
                    update_weight = weights[advisor_i][provider_i] * math.exp(-learning_rate*loss_advisors[advisor_i][provider_i])
