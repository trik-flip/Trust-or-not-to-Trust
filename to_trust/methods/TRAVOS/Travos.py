...
class Travos(Consumer):
    ...
    def register_witnesses(self, witnesses: list[Witness]):
        ...
        for witness in witnesses:
            ...
            for provider in self.providers:
                self._outcome_history[witness][provider] = np.array(
                    [],
                    dtype=[
                        ("run_index", int),
                        ("outcome", bool),
    ...
    def update_provider(self, provider: Provider, score: float) -> None:
        ...
        for witness in self.witnesses:
            relevant_entry = np.argwhere(
                self._outcome_history[witness][provider]["run_index"] == self.run_index
            )
            expected_outcome = (
                self._outcome_history[witness][provider][relevant_entry][
                    "expected_value"
                ]
                > self._outcome_threshold
            )
            self._outcome_history[witness][provider][relevant_entry]["outcome"] = (
    ...
    def choose_provider(self) -> Provider:
        ...
        for provider, (confidence_value, expected_value) in confidence_values.items():
            ...
            if confidence_value < self._confidence_threshold:
                expected_value = self._estimate_value_with_witnesses_opinions(
    ...
    def _confidence_values_of_providers(self) -> dict[Provider, (float, float)]:
        ...
        for p in self.providers:
            expected_value = stats.beta.expect(
                args=(self._alphas_p[p], self._betas_p[p])
            )
    ...
    def _estimate_value_with_witnesses_opinions(
        self, provider: Provider, original_expected_value: float
    ):
        ...
        beta = 0
        for witness in self.witnesses:
            witness_accuracy, witness_expected_value = self._accuracy_of_witness(
                provider, witness, bin_index
            )
    ...
    def _accuracy_of_witness(
        self, provider: Provider, witness: Witness, bin_idx: int
    ) -> (float, float):
        # calculate accuracy of witness
        alpha = 1
        beta = 1

        relevant_entries = np.argwhere(
            self._outcome_history[witness][provider]["bin_index"] == bin_idx
        )
        alpha += np.sum(
            self._outcome_history[witness][provider][relevant_entries]["outcome"]
        )
        beta += np.sum(
            np.invert(
                self._outcome_history[witness][provider][relevant_entries]["outcome"]
        ...
        self._outcome_history[witness][provider] = np.append(
            np.array(
                [(self.run_index, False, expected_value, bin_idx)],
                dtype=[
                    ("run_index", int),
                    ("outcome", bool),
                    ("expected_value", float),
