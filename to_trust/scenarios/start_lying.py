...
from to_trust import Scenario, Witness, Consumer, Provider


class StartLyingScenario(Scenario):
    ...
    def __init__(
        ...
        for _ in range(witnesses_to_lie):
            witnesses.append(Witness(honesty=1, starts_lying=True, epochs_before_dishonest=honest_epochs))
        for _ in range(witness_amount-witnesses_to_lie):
            witnesses.append(Witness(honesty=1))

        super().__init__(witnesses=witnesses, consumers=consumers, consumer_amount=consumer_amount, consumer_options=consumer_options, providers=providers, provider_amount=provider_amount, provider_options=provider_options)
